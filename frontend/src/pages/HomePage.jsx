  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center justify-center bg-white font-fredoka">
      ...
      <div className="w-full flex flex-col items-center justify-center py-4">
        {formattedData ? (
          <div className="bg-green-100 border border-green-400 text-green-800 px-6 py-4 rounded shadow w-fit">
            <strong>Live Sensor Dashboard</strong>

            {/* Graph Selector Buttons */}
            <div className="flex flex-wrap gap-2 mt-4 mb-6 min-w-3xl justify-center">
              {sensorKeys.map((key) => (
                <button
                  key={key}
                  className={`px-3 btn-anim py-1 rounded border font-semibold transition-colors ${
                    selectedGraph === key
              ...
              <div className="bg-white border border-gray-200 rounded p-3 shadow w-full">
                <div className="flex items-center mb-2">
                  <span className="font-semibold text-gray-700 mr-2">
                    {variableConfig[selectedGraph].label}
                  </span>
                  <span className="text-xs text-gray-500">
                    ({variableConfig[selectedGraph].unit})
                  </span>
                  <span className="ml-auto text-sm text-gray-500">
                    Current:{" "}
                    <span className="font-mono text-green-700">
                      {formattedData.values[selectedGraph]?.value ?? "--"}
                    </span>
                  </span>
                </div>
                <Line
                    ...
                    datasets: [
                      {
                        ...
                        backgroundColor:
                          variableConfig[selectedGraph].color + "33",
                        fill: true,
                        tension: 0.3,
                        pointRadius: 0,
                      },
            ...
            {/* All Sensors Graph */}
            {selectedGraph === "all" && (
              <div className="mt-6 flex min-w-5xl gap-10 items-center justify-center flex-row">
                <div className="flex-10 bg-white border border-gray-200 rounded p-3 shadow w-full">
                  <div className="flex items-center mb-2">
                    <span className="font-semibold text-gray-700 mr-2">
                      All Sensors
                    </span>
                  ...
                  <Line
                    ...
                    options={{
                      ...
                      scales: {
                        ...
                        y: {
                          min: Math.min(
                            ...sensorKeys.map((k) => variableConfig[k].min)
                          ),
                          max: Math.max(
                            ...sensorKeys.map((k) => variableConfig[k].max)
                          ),
                          ticks: {
                            color: "#888",
                            callback: function (value) {
                ...
                <div className="mt-6 flex-1 flex justify-center">
                  <table className="min-w-[320px] max-w-lg w-full bg-white border border-gray-300 rounded-lg shadow text-sm">
                    <thead>
                      <tr className="bg-green-100">
                        <th className="px-4 py-2 text-left font-semibold text-green-800">
                          Sensor
                        </th>
                        <th className="px-4 py-2 text-left font-semibold text-green-800">
                          Value
                    ...
                    <tbody>
                      {sensorKeys.map((key) => (
                        <tr key={key} className="even:bg-green-50">
                          <td className="px-4 py-2 font-medium text-gray-700">
                            {variableConfig[key].label}
                          </td>
                          <td className="px-4 py-2 font-mono text-green-700">
                            {formattedData.values[key]?.value ?? "--"}
                          </td>
                          <td className="px-4 py-2 text-gray-500">
                            {variableConfig[key].unit}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="mt-4 pt-4 border-t border-green-300 text-sm">
              <div>
                <strong>Device:</strong> {formattedData.metadata.device}
              </div>
              <div>
                <strong>Last Update:</strong> {formattedData.metadata.timestamp}
              </div>
            </div>
